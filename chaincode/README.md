...
**Norma aplicable**: el destino de una unidad dispuesta finalmente es un residuo peligroso — **Ley 24.051, Anexo I, categoría Y3** («Desechos de medicamentos y productos farmacéuticos»), [texto oficial](https://www.argentina.gob.ar/normativa/nacional/450/actualizacion) —, lo que obliga a documentar la destrucción y atribuirla a un responsable. El contrato lo satisface con el `motivo` obligatorio y con el `custodioActual`, que la operación no mueve. El chaincode **no** valida el contenido del `motivo`: exigir que cite una norma sería una condición de rechazo que ningún ADR decide, y un texto libre no es verificable.
