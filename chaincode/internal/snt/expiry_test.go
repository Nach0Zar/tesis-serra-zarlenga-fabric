...
// issue "unidad vencida bloqueada para dispensacion", que no necesita regla
// propia: ADR-001 no declara T06 desde VENCIDO.
func TestReportExpiredFromCustodyBlocksDispensing(t *testing.T) {
	...
	// Y tampoco el propio custodio, que es el caso que importa: el bloqueo es
	// del ESTADO, no de la identidad.
	_, err = contract.Dispense(
