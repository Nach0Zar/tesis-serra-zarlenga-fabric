...
> - **Octava ronda de review del PR #88 (2026-08-19)**: dos P1 sobre las issues y tres P2 sobre residuos. **Comentarios rectores obsoletos** en las issues de decisión (#81–#87): describen decisiones descartadas —`memberOnlyWrite` y generación desde el registro del ledger, `OR(AND(emisor,receptor), AnmatMSP)`, «Fabric CA única», «todos los peers ejecutan el mismo binario», el `mspId` regulatorio como parámetro del lifecycle, la firma de `VerifyTrace` diferida a CC-8, el esquema de cuatro tablas—. Se resuelven con un comentario nuevo por issue que declare superado el anterior, sin editar el histórico. **Obligaciones nuevas no propagadas a las issues de implementación**: se actualizaron #16 (re-evaluación del par en la recepción y reintento por dato privado no diseminado), #23 (construcción única del paquete y verificación `queryinstalled`/`queryapproved`), #25 (demostrar que una matriz alterada impide completar la recepción), #30 (`ReturnOp`), #37 (snapshot completo en `unit_events`, `return_operations`, `REG:`, índice por GTIN) y #38 (contrato v2.6.1 y superficie completa); y se aplicaron las de la ronda anterior en #27, #28 y #39. **Residuos**: ADR-010 corregía el `packageID` en el cuerpo pero sus referencias y su punto de `go:embed` seguían afirmando lo contrario, y ADR-008 lo repetía en su alternativa B; ADR-012 decía «cuatro tablas» con cinco; ADR-002 volvía a derivar la granularidad a NET-5 justo después de aclarar que ADR-006 la había fijado; y README, `chaincode/README.md`, `client/README.md` y el roadmap seguían en v2.1.0. **`requiredPeerCount: 1` no garantiza que el receptor tenga la PDC al confirmarse el bloque** —el único peer alcanzado puede ser el de ANMAT—, con lo que la recepción puede fallar de forma transitoria hasta que el *pull* o la reconciliación completen. Se conserva el valor, se declara el trade-off frente a `requiredPeerCount: 2` (que pondría el despacho, camino medido por DES-7, a depender de dos peers más) y se derivan las obligaciones a NET-5 y CC-3. **Nota sobre la premisa del primer hallazgo**: la review lo funda en que «AGENTS.md prioriza issues y comentarios sobre los ADR», pero ese archivo **no existe en el repositorio** en ninguna rama. El hallazgo se acepta igual por su propio mérito —un comentario rector obsoleto induce a implementar el diseño viejo—, y conviene decidir si esa regla de precedencia debe existir y escribirse.
