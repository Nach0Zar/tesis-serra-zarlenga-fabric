...
| Transferencia ordinaria de custodia | Custodio actual activo, destino activo y par permitido por DES-3 | Origen y destino de la transferencia. ADR-004 (DES-9) separo despacho y recepcion, y la transaccion que confirma el cambio de custodia queda cubierta por ambas partes. **Materializacion** (ADR-007, punto 6.b): el despacho fija sobre la clave de la unidad la politica `AND(emisor, receptor declarado)`, **sin rama alternativa**. Ninguna otra organizacion —tampoco `AnmatMSP`— puede sustituir a una de las dos partes en la recepcion, el rechazo o cualquier otra escritura durante el transito. |
...
- incluir a la organizacion regulatoria en la politica de endoso de chaincode,
  junto con las custodiales: esa politica rige la primera escritura de una clave
  publica, y dos de esas primeras escrituras —el alta de una entrada del registro
  y la creacion de una autorizacion de intervencion— son actos que esta tabla
  reserva a la autoridad; una politica que solo enumere a las custodiales exige
  para ellos el endoso accidental de una custodial cualquiera (ADR-007, punto
  6.j);
- acompanar **toda** creacion de una clave publica nueva con el marcador de
  participacion de la organizacion responsable de esa escritura: es lo unico que
  la exige mientras la clave no tenga politica propia, y sin esa invariante la
  politica de chaincode volveria a ser la unica barrera;
- no confundir un campo de vigencia con un mecanismo de revocacion: dejar sin
  efecto una autorizacion vigente exige una operacion propia
  (`RevokeLabIntervention`), porque el vencimiento no borra la clave ni su
  politica (ADR-007, punto 6.f);
