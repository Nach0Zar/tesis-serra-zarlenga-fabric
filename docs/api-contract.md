...
- **Comprobaciones**, evaluadas en orden: existencia de la unidad; unicidad (una sola creación de la clave en el historial, sin recreación); cadena de custodia legítima (camino de estados válido contra ADR-001 y cada cambio de custodio autorizado por la matriz de DES-3); y estado actual apto para operar (ni bloqueante ni terminal según ADR-001).
...
- **Valores de `motivo`**: `NO_ENCONTRADA`, `UNIDAD_DUPLICADA`, `SECUENCIA_INVALIDA`, `TRANSFERENCIA_NO_AUTORIZADA`, `ESTADO_BLOQUEANTE`, `ESTADO_TERMINAL`; vacío cuando `autentica` es `true`. `SECUENCIA_INVALIDA` y `TRANSFERENCIA_NO_AUTORIZADA` son los mismos veredictos que `VerifyTrace`, con idéntica semántica: la comprobación de cadena está implementada una sola vez y ambas operaciones la consumen (ADR-013, Decisión, comprobación 3).
- **Valores de `resultado`**: `OK`, `FALLO`, `NO_EVALUADO` (comprobaciones posteriores a la que falló).
