...
- **Norma aplicable a la disposición final, y qué parte de ella cubre el prototipo**: el destino de una unidad dispuesta finalmente es un residuo peligroso. La **Ley 24.051, Anexo I, categoría Y3** («Desechos de medicamentos y productos farmacéuticos») lo clasifica como tal. URL oficial: https://www.argentina.gob.ar/normativa/nacional/450/actualizacion.
  Esa ley **no** queda satisfecha por esta operación, y el contrato no lo afirma: su régimen exige un **manifiesto** que acompañe al residuo identificando generador, transportista y operador de tratamiento o disposición, con la naturaleza y la cantidad de lo transportado y su origen y destino. El prototipo no modela el transporte de residuos ni esos tres actores —ninguno es una organización del SNT— y por lo tanto no emite manifiesto alguno. Es un límite de alcance declarado, no una omisión: ver [`docs/alcance-prototipo.md`](alcance-prototipo.md).
  Lo que la operación **sí** aporta, y es lo que el criterio de EXT-8 (#63) pide del chaincode, son los dos datos que vuelven atribuible el hecho: el `motivo` —argumento público obligatorio, donde viaja la referencia normativa y el acto administrativo que respalda la destrucción— y el `custodioActual`, que la operación **no** mueve, de modo que el responsable registrado de la unidad al momento de la disposición queda en la traza inmutable. El contrato **no** valida el contenido del `motivo`: exigir que cite una norma sería una condición de rechazo nueva, sin ADR que la decida, y un texto libre no es verificable por el chaincode.
