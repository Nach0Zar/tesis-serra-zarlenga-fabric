...
- **Norma aplicable a la disposición final**: el destino de una unidad dispuesta finalmente es un residuo peligroso. La **Ley 24.051, Anexo I, categoría Y3** («Desechos de medicamentos y productos farmacéuticos») lo clasifica como tal, lo que obliga a que la destrucción quede documentada y atribuida a un responsable. URL oficial: https://www.argentina.gob.ar/normativa/nacional/450/actualizacion. El contrato la satisface por dos vías, ambas ya existentes y ninguna nueva: el `motivo` —argumento público obligatorio, donde viaja la referencia normativa y el acto que la respalda— y el `custodioActual`, que la operación **no** mueve, de modo que el responsable registrado de la unidad al momento de la disposición queda en la traza. El contrato **no** valida el contenido del `motivo`: exigir que cite una norma sería una condición de rechazo nueva, sin ADR que la decida, y un texto libre no es verificable por el chaincode.
