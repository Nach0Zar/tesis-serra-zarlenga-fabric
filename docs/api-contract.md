...
- **`DISPUESTO_FINAL` es terminal**: [ADR-001](adr/001-maquina-estados-medicamento.md) no declara ninguna transición con ese estado de origen, de modo que **toda** operación de escritura sobre la unidad devuelve después `INVALID_STATE_TRANSITION` — transferencia, dispensación, eventos extraordinarios y la propia `FinalDisposition` incluidas. No hay un código distinto para «unidad ya dispuesta»: el estado terminal se expresa con el mismo rechazo de transición que el resto de la máquina.
