...
- **Historial de correcciones compatibles**: `2.7.1` — normaliza el resultado
  `newest-first` de `GetHistoryForKey` al orden cronológico que ya exigían
  `GetUnitHistory` y las verificaciones de custodia. Corrige el falso
  `SECUENCIA_INVALIDA` observado por NET-6 sobre trazas legítimas con más de una
  modificación. No cambia firmas, tipos, esquemas ni `code`s; es **PATCH**.
