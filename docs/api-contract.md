...
Las operaciones que modifican una unidad emiten **exactamente un** evento de
chaincode cuando la transacción queda confirmada como válida. El nombre del
evento coincide con el nombre PascalCase de la operación y el payload es
exactamente la `MedicationUnitView` pública resultante: no contiene
`transient`, datos de PDC ni campos adicionales.

| Operaciones que emiten un evento |
|---|
| `RegisterUnit`, `DispatchTransfer`, `ReceiveTransfer`, `RejectTransfer`, `Dispense` |
| `Quarantine`, `ReleaseQuarantine`, `ReportExpired`, `ReportStolen`, `ReportLost`, `ReportDamaged` |
| `WithdrawFromMarket`, `ProhibitProduct`, `ReturnProduct`, `Restock`, `FinalDisposition` |

No emiten eventos:

- `Init`, `AuthorizeLabIntervention`, `RevokeLabIntervention`,
  `RegisterOrganization` y `SetOrganizationActive`;
- `ReadUnit`, `GetUnitHistory`, `GetLabInterventionHistory`, `QueryUnitsByGTIN`,
  `QueryUnitsByState`, `VerifyUnit` y `VerifyTrace`.

La observabilidad distingue tres resultados:

1. Una **transacción válida confirmada** de las operaciones del catálogo
   anterior produce el evento de negocio. Fabric Gateway entrega
   `ChaincodeEvents` solo después del commit válido.
2. Una **transacción incluida en un bloque pero invalidada** no produce un
   evento de negocio confirmado. Se observa en los bloques filtrados del canal
   mediante su `TxValidationCode` (por ejemplo,
   `ENDORSEMENT_POLICY_FAILURE` o `MVCC_READ_CONFLICT`).
3. Un **rechazo durante la propuesta** no llega al servicio de ordenamiento ni
   al ledger. Por lo tanto no existe bloque, código de validación ni evento
   on-ledger que un listener pueda recibir; el cliente que presentó la
   propuesta es quien recibe el error contractual o de endoso.

El listener regulatorio y su formato operativo se documentan en
[`anmat-event-listener.md`](anmat-event-listener.md). Este catálogo no cambia
transiciones, autorización, endoso, PDC ni respuestas de las funciones.
...
- **Errores**: `UNIT_NOT_FOUND`, `NOT_IN_TRANSIT`, `RECEIVER_MISMATCH` (el invocador no es ni el destinatario declarado ni el emisor), `UNAUTHORIZED_ROLE`, `ORG_NOT_REGISTERED`, `ORG_INACTIVE`, `INVALID_REQUEST`.
...
- **Autorización**: invocador = custodio actual con `agentType=PHARMACY` o `HEALTHCARE_FACILITY`, `snt.role=operator`. Como toda operación custodial, exige además que la organización tenga entrada en el registro de ADR-003 y esté habilitada (`active=true`): esas dos condiciones producen `ORG_NOT_REGISTERED` y `ORG_INACTIVE`.
...
| `WithdrawFromMarket` | T17, T18, T19 | `RETIRADO_MERCADO` | ANMAT o laboratorio titular; **desde `EN_TRANSITO`, solo ANMAT** (ADR-001 rev. 2, DES-19) | Peer del custodio actual; + peer regulatorio cuando lo inicia ANMAT; **laboratorio no custodio: previa `AuthorizeLabIntervention` + peers del laboratorio y del regulador** (DES-6; ADR-007, punto 6.e) |
...
| `FinalDisposition` | T28, T29, T30, T31, T32, T33 | `DISPUESTO_FINAL` | Agente de recupero/disposición (T28, T29, T33, y también T31/T32); **+ ANMAT** en T30, T31 y T32; **+ laboratorio titular** solo en T31 (desde `RETIRADO_MERCADO`) | Peer del custodio actual; + peer regulatorio en disposiciones regulatorias; laboratorio no custodio: previa `AuthorizeLabIntervention` + peers del laboratorio y del regulador |
...
- **Intervención de un laboratorio no custodio** (`WithdrawFromMarket`, `Restock`, `FinalDisposition`): cuando el invocador es una organización con `agentType=LABORATORY` que **no** es el custodio actual, el chaincode exige una autorización de intervención en `estado=ACTIVA` y no vencida (`AuthorizeLabIntervention`) para esa unidad, ese laboratorio y esa operación, la marca como `CONSUMIDA` y escribe **marcadores de participación** en la colección implícita del laboratorio y en la de la organización regulatoria. La plataforma exige, en consecuencia, el endoso del laboratorio designado y el de la organización regulatoria —el par que pide DES-6— más el del custodio actual que impone la clave de la unidad: tres organizaciones (ADR-007, punto 6.e). Sin autorización, o con una vencida, ya consumida o revocada, `LAB_INTERVENTION_REQUIRED`. Este camino solo existe en estados de **reposo**: desde `EN_TRANSITO` el retiro está reservado a ANMAT, porque salir de ese estado obliga a cerrar el registro de la operación en la colección privada del par y ADR-006 (punto 1) limita su membresía a `{emisor, receptor, regulador}` (ADR-001 rev. 2, DES-19). Un laboratorio ajeno al par recibe `INVALID_STATE_TRANSITION`, que es un rechazo de la tabla de ADR-001 y no de la autorización.

- **`Restock` mira también la fecha**: el reingreso a stock se rechaza cuando `fechaVencimiento` ya pasó, aunque el evento `INFORMAR_VENCIMIENTO` no se haya registrado y el estado siga siendo `DEVUELTO`, `EN_CUARENTENA` o `RETIRADO_MERCADO`. Es la condición de aptitud que enumera [ADR-009](adr/009-return-and-recovery-semantics.md) punto 5 y el único de sus términos que la tabla de transiciones no expresa por sí sola: los demás —destruida, robada, extraviada, prohibida, dispuesta final— son estados desde los que ADR-001 no declara `REINGRESAR_STOCK`. Una unidad inmovilizada o devuelta puede alcanzar la fecha mientras espera, y sin esta comprobación el reingreso la devolvería al circuito de dispensación ya vencida. Comparte comparación e implementación con `Dispense` y `VerifyUnit` —`fechaVencimiento` es el último día operable, contra la fecha calendario UTC de `GetTxTimestamp()`— y el rechazo usa `INVALID_STATE_TRANSITION` con `details.causa = "VENCIDO_POR_FECHA"`. El camino correcto para esa unidad es `ReportExpired` y después `FinalDisposition`.
- **`DISPUESTO_FINAL` es terminal**: [ADR-001](adr/001-maquina-estados-medicamento.md) no declara ninguna transición con ese estado de origen. La garantía es exactamente esa —**la unidad no puede salir de ese estado**— y no que toda escritura devuelva un mismo código: cada operación valida lo suyo antes de llegar a la máquina de estados, y devuelve la primera condición que falla.
  - Las trece operaciones que llegan a la transición (`DispatchTransfer`, `Dispense`, los eventos extraordinarios, `Restock` y `FinalDisposition`): `INVALID_STATE_TRANSITION`. No hay un código distinto para «unidad ya dispuesta».
  - `ReceiveTransfer` y `RejectTransfer`: `NOT_IN_TRANSIT`, porque comprueban antes que la unidad esté `EN_TRANSITO`.
  - `RegisterUnit`: `UNIT_ALREADY_EXISTS`. No intenta una transición, intenta crear una clave que ya existe — y es lo que impide reciclar una unidad dispuesta registrándola de nuevo.
  - `AuthorizeLabIntervention` **no falla**, y es un límite declarado del prototipo: la autoridad puede emitir una autorización de intervención sobre una unidad ya dispuesta. Es inocua, porque ejercerla exigiría una transición que ADR-001 no declara y la autorización nunca puede consumirse, pero deja un asiento que promete algo irrealizable. Bloquearla sería una regla que ninguna ADR decide hoy.
- **Norma aplicable a la disposición final, y qué parte de ella cubre el prototipo**: el destino de una unidad dispuesta finalmente es un residuo peligroso. La **Ley 24.051** lo clasifica como tal en su **Anexo I, categoría Y3** («Desechos de medicamentos y productos farmacéuticos») y su régimen impone, en los **arts. 12 a 14**, que el residuo circule acompañado de un **manifiesto** que identifique a los sujetos intervinientes —generador, transportista y planta de tratamiento o disposición final— junto con la naturaleza y cantidad del residuo y su origen y destino; el **art. 14** define al *generador* como quien produce el residuo como resultado de sus actos o de cualquier proceso u operación. URL oficial: https://www.argentina.gob.ar/normativa/nacional/450/actualizacion.
  Ese régimen **no** queda satisfecho por esta operación, y el contrato no lo afirma. La ausencia hay que enunciarla con precisión, porque los tres sujetos no están en la misma situación:
  - El **generador** puede ser perfectamente una organización del SNT: por la definición del art. 14, quien produce el residuo es quien tiene la unidad al disponerla, es decir el custodio actual —farmacia, droguería o laboratorio—. La operación **sí** lo deja registrado, porque no mueve el `custodioActual`.
  - El **transportista** y la **planta de tratamiento o disposición** no están modelados: no son organizaciones de la cadena de trazabilidad de medicamentos que [ADR-002](adr/002-topologia-canales.md) define, y el prototipo no representa el traslado del residuo.
  - El **manifiesto** de los arts. 12-13, como documento, no se emite ni se persiste.
  Es un límite de alcance declarado, no una omisión: ver [`docs/alcance-prototipo.md`](alcance-prototipo.md).
  Lo que la operación **sí** aporta, y es lo que el criterio de EXT-8 (#63) pide del chaincode, son los dos datos que vuelven atribuible el hecho: el `motivo` —argumento público obligatorio, donde viaja la referencia normativa y el acto administrativo que respalda la destrucción— y el `custodioActual`, que la operación **no** mueve, de modo que el responsable registrado de la unidad al momento de la disposición —el generador, en los términos del art. 14— queda en la traza inmutable. El contrato **no** valida el contenido del `motivo`: exigir que cite una norma sería una condición de rechazo nueva, sin ADR que la decida, y un texto libre no es verificable por el chaincode.
- **Quién dispone finalmente según el origen**: ADR-001 no le da el mismo actor a las seis filas, y la diferencia es sustantiva. T28 (`VENCIDO`), T29 (`DETERIORADO`) y T33 (`DEVUELTO`) habilitan **únicamente** al agente de recupero/disposición, que [ADR-009](adr/009-return-and-recovery-semantics.md) punto 3 resuelve como el custodio actual registrado: la unidad está físicamente en su poder y es él quien ejecuta y documenta la destrucción. ANMAT se suma en T30, T31 y T32; el laboratorio titular solo en T31. En T32 (`PROHIBIDO`) el titular **no** aparece, en coherencia con `ProhibitProduct`: la prohibición es una potestad exclusivamente regulatoria y su cierre no queda en manos del titular del producto. Del mismo modo, desde `RETIRADO_MERCADO` el custodio puede disponer finalmente la unidad (T31) pero **no** reingresarla a stock (T27).
...
- **Response**: lista de entradas del historial en orden cronológico, desde la
  modificación más antigua hacia la más reciente. El chaincode normaliza el
  orden `newest-first` que `GetHistoryForKey` entrega desde Fabric 2.x:
...
```go
func (c *SNTContract) GetLabInterventionHistory(ctx contractapi.TransactionContextInterface, gtin string, numeroSerie string) ([]LabInterventionHistoryEntry, error)
```

Consulta la clave pública `LabIntervention`+[`gtin`,`numeroSerie`] mediante
`GetHistoryForKey`, sin índice ni segunda fuente de verdad. Es una lectura
pública como `GetUnitHistory`: no resuelve identidad, no escribe estado,
marcadores ni eventos y no altera SBE. Devuelve solo modificaciones
confirmadas, en orden cronológico de la más antigua a la más reciente.

- **Response**: lista de `LabInterventionHistoryEntry` con `txId`,
  `timestamp` UTC RFC3339, `isDelete` y el valor completo de
  `LabInterventionView` en cada modificación. En una eliminación, `value`
  es `null`.
- **Ciclo**: una nueva autorización reemplaza el valor de la misma clave;
...
```go
func (c *SNTContract) VerifyUnit(ctx contractapi.TransactionContextInterface, gtin string, numeroSerie string) (*UnitVerdict, error)
```

Verificación de autenticidad de una unidad **antes de adquirirla**, con la semántica que fija [ADR-013](adr/013-acquirer-authenticity-verification.md). Materializa la obligación que la Disposición ANMAT 3683/2011 impone a todo miembro de la cadena que adquiere un medicamento: poder consultar si las unidades son originales y fueron distribuidas por la cadena legalmente autorizada.

**No se confunde con `VerifyTrace`**, aunque compartan dos comprobaciones. `VerifyTrace` audita una unidad **ya dispensada** como condición de pago del financiador y devuelve `NO_DISPENSADA` para cualquier otro estado; `VerifyUnit` se invoca **antes de aceptar la custodia**, cuando la unidad está justamente en `EN_TRANSITO` o `EN_CUSTODIA`, y pregunta si es apta para operar.

- **Autorización**: invocador registrado y habilitado (ADR-003). **No** exige `agentType` ni `snt.role`: el consultante es cualquier eslabón de la cadena que esté por adquirir, y restringirlo sería una barrera aparente, porque la misma información es alcanzable con `ReadUnit` y `GetUnitHistory` (ADR-005). No muta estado.
- **Comprobaciones**, evaluadas en orden: existencia de la unidad; unicidad (una sola creación de la clave en el historial, sin recreación); **cadena de custodia legítima**, que verifica de forma conjunta las invariantes de ADR-001 y ADR-004 —primer estado `EN_LABORATORIO` con custodio de `agentType=LABORATORY`, cada par consecutivo de estados es una transición declarada (incluidas las escrituras que no cambian el estado, que ADR-001 no declara), `CustodioActual` cambia si y solo si la transición es `EN_TRANSITO → EN_CUSTODIA`, y ese cambio está autorizado por la matriz de DES-3—; y **aptitud para operar**, que rechaza el estado terminal, el estado bloqueante y la unidad cuya `fechaVencimiento` ya pasó aunque el evento `INFORMAR_VENCIMIENTO` no se haya registrado.
- **Vencimiento por fecha**: `fechaVencimiento` es el último día operable. La unidad está vencida cuando la fecha calendario UTC de `GetTxTimestamp()` es **estrictamente posterior** a ella. El instante sale siempre del timestamp de la transacción y nunca del reloj local, para que el veredicto sea determinístico entre peers y reproducible por un auditor.
- **Response** (`UnitVerdict`): `autentica` es `true` solo si las cuatro comprobaciones pasan; `motivo` nombra la primera que falla; `estado` es el estado público observado.
...
```json
{
  ...
  "verificaciones": [
    { "check": "EXISTENCIA", "resultado": "OK", "detalle": "" },
    { "check": "UNICIDAD", "resultado": "OK", "detalle": "" },
    { "check": "CADENA_CUSTODIA", "resultado": "OK", "detalle": "" },
    { "check": "ESTADO_OPERABLE", "resultado": "FALLO", "detalle": "RETIRADO_MERCADO" }
  ]
}
```

- **Valores de `motivo`**: `NO_ENCONTRADA`, `UNIDAD_DUPLICADA`, `SECUENCIA_INVALIDA`, `TRANSFERENCIA_NO_AUTORIZADA`, `ESTADO_TERMINAL`, `ESTADO_BLOQUEANTE`, `VENCIDO_POR_FECHA`; vacío cuando `autentica` es `true`. `SECUENCIA_INVALIDA` y `TRANSFERENCIA_NO_AUTORIZADA` son los mismos veredictos que `VerifyTrace`, con idéntica semántica: la comprobación de cadena está implementada una sola vez y ambas operaciones la consumen (ADR-013, Decisión, comprobación 3).
- **Valores de `resultado`**: `OK`, `FALLO`, `NO_EVALUADO` (comprobaciones posteriores a la que falló).
- **Nota**: la inexistencia de la unidad **no** es un error sino el veredicto `NO_ENCONTRADA`, por el mismo criterio que en `VerifyTrace`: para quien consulta antes de adquirir es una respuesta legítima —y la más importante—, no una falla de invocación.
- **Límites declarados** (ADR-013): la verificación acredita la traza registrada, **no** la autenticidad material del envase ni la ausencia de clonación del serial —un envase falsificado que reproduzca un GTIN + serie legítimo obtiene `autentica: true`—; la comprobación de unicidad detecta una recreación de la clave, no dos envases físicos con el mismo serial; no valida la habilitación histórica de los actores ni distingue versiones históricas de la matriz; y un veredicto `autentica: true` **no** es una autorización de recepción, que sigue decidiendo `ReceiveTransfer` contra el registro de operación privado.
- **Errores**: `INVALID_REQUEST`, `ORG_NOT_REGISTERED`, `ORG_INACTIVE`.
...
// LabInterventionHistoryEntry
{
  "txId": "string",
  "timestamp": "string (ISO 8601 UTC)",
  "isDelete": "boolean",
  "value": "LabInterventionView | null"
}
```
...
- **Corrección PATCH `2.11.2`**: declara **opcional en la metadata legible por máquina** el campo `value` de `UnitHistoryEntry` y `LabInterventionHistoryEntry`, que el contrato documenta como `null` en una entrada de **borrado**. Alcance preciso, porque la versión anterior de esta nota afirmaba de más: **la respuesta ya era válida** — el servidor aceptaba `value: null` con el campo en `required`, porque la clave está presente y el schema referenciado declara `properties`, `required` y `additionalProperties` pero **no** `type: "object"`, de modo que un `null` los satisface vacuamente. Lo que se corrige es lo que la metadata le **dice a un consumidor**: declarar obligatorio un campo documentado como nulo lleva a que un cliente generado desde ese schema lo modele como no opcional y falle al recibir una entrada de borrado. Se agrega además una regresión que ejerce el schema compilado y la respuesta serializada, para que la promesa de `value: null` quede protegida si una versión futura de la librería emitiera el `type`. No cambian firmas, campos, tipos, `code`s ni semántica: **PATCH**, con el mismo criterio que la `2.11.1`.
- **Corrección PATCH `2.11.1`**: alinea la metadata legible por máquina que genera Fabric Contract API con el esquema ya congelado de `LabInterventionView`. Los campos condicionales `consumidaEn`, `revocadaEn` y `motivoRevocacion` se declaran opcionales también en esa metadata; el JSON documentado ya establecía que solo aparecen en estado `CONSUMIDA` o `REVOCADA`, pero `json:",omitempty"` no era suficiente para que Contract API los excluyera de `required`. No cambian firmas, campos, tipos ni semántica; los clientes que consumen el schema dejan de recibir como obligatorios tres campos que una respuesta `ACTIVA` no contiene. La metadata generada forma parte de la superficie observable y por eso la corrección queda registrada como **PATCH**.
- **Cambio compatible `2.11.0`**: CC-10 (#123) agrega `GetLabInterventionHistory`, consulta pública de la clave de intervención con los snapshots confirmados en orden cronológico. Además unifica la semántica de integridad del historial que consumen `GetUnitHistory`, `VerifyUnit` y `VerifyTrace`: una entrada no borrada vacía o corrupta produce `INTERNAL_ERROR` en vez de omitirse silenciosamente durante la verificación de la cadena. Las rutas vigentes del chaincode no pueden crear esa anomalía porque `putUnit` siempre serializa el snapshot completo; la regla cubre corrupción del ledger o un escritor externo. No cambian firmas, tipos, significado de `code`s, estados ni endoso, y la nueva operación hace que la versión sea **MINOR**. La paridad relacional se define en ADR-012 revisión 3 y no agrega una ronda cuantitativa a M4.
- **Cambio compatible `2.10.0`**: incorpora el catálogo de eventos de chaincode requerido por NET-8 (#64): enumera las dieciséis operaciones que emiten exactamente un evento con el nombre de la operación y payload `MedicationUnitView`, las once operaciones que no emiten y la separación observable entre transacciones válidas confirmadas, transacciones incluidas pero invalidadas y propuestas rechazadas que nunca llegan al ledger. Es un agregado compatible de superficie observable, sin cambios de firmas, `code`s, esquemas, transiciones, roles, endoso ni PDC; por eso se registra como **MINOR**.
- **Historial de cambios compatibles**: `2.9.2` — declara lo que EXT-8 (#63) necesitaba explícito para `FinalDisposition`, en tres puntos. **(1)** Precisa la columna de **actor habilitado**, que decía «agente de recupero/disposición, ANMAT o laboratorio según origen» sin decir según qué origen: T28, T29 y T33 habilitan únicamente al agente de recupero —que [ADR-009](adr/009-return-and-recovery-semantics.md) punto 3 resuelve como el custodio actual—, ANMAT se suma en T30, T31 y T32, y el laboratorio titular solo en T31; en T32 el titular no aparece, en coherencia con `ProhibitProduct`. **(2)** Declara la terminalidad de `DISPUESTO_FINAL` con la garantía que ADR-001 realmente da —ninguna transición declara ese estado de origen, es decir que la unidad no puede salir de él— y enumera el código de cada grupo de operaciones, en lugar de prometer un código uniforme que la implementación no sostiene; incorpora como límite declarado que `AuthorizeLabIntervention` puede emitir una autorización inconsumible sobre una unidad ya dispuesta. **(3)** Agrega la trazabilidad normativa de la disposición final —**Ley 24.051**, Anexo I categoría Y3 y **arts. 12 a 14**, con URL oficial— distinguiendo los tres sujetos del manifiesto: el **generador** (art. 14) es el custodio actual y queda registrado, mientras que el transportista, la planta de tratamiento y el manifiesto como documento quedan fuera de alcance (`docs/alcance-prototipo.md`); y explicita que el contrato no valida el contenido de `motivo`. Sin cambios de firmas, `code`s ni esquemas: **PATCH**. `2.9.1` — declara la condición de aptitud por fecha de `Restock`, que EXT-5 (#31) implementa: el reingreso a stock se rechaza cuando `fechaVencimiento` ya pasó aunque el evento no se haya registrado, con `INVALID_STATE_TRANSITION` y `details.causa = "VENCIDO_POR_FECHA"` — el mismo código y la misma causa que ya usa `Dispense`, con la que comparte implementación. Es el único término de la aptitud de ADR-009 punto 5 que la tabla de ADR-001 no expresa por sí sola. No se agrega ningún `code` al catálogo, no cambian firmas ni esquemas y ninguna operación previamente implementada cambia de comportamiento: se registra como **PATCH**, con el criterio de la `2.6.2` — el contrato deja de omitir una condición observable de una operación que ya declaraba. `2.9.0` — incorpora la operación de
