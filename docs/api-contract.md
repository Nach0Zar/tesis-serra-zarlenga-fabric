...
- **Corrección PATCH `2.11.2`**: extiende la corrección de la `2.11.1` a los dos tipos de historial. `UnitHistoryEntry` y `LabInterventionHistoryEntry` declaraban `value` como **obligatorio** en la metadata legible por máquina, mientras el propio contrato promete —para `GetUnitHistory` y `GetLabInterventionHistory`— que en una entrada de **borrado** ese campo es `null`. El documento y el schema que el peer expone se contradecían: la respuesta que el contrato describe habría sido rechazada al serializarse. La condición es hoy **inalcanzable**, porque ninguna ruta del chaincode borra la clave de la unidad ni la de intervención; lo que se corrige no es un comportamiento sino la contradicción, que existe aunque nadie la ejerza. No cambian firmas, campos, tipos, `code`s ni semántica: **PATCH**, con el mismo criterio que la `2.11.1`.
