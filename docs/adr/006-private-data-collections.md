...
- Para cada par (organización origen, organización destino) cuyo par de `agentType` esté autorizado por la matriz de DES-3, se define una colección explícita con membresía {org origen, org destino, `AnmatMSP`} y nombre determinístico `transfer_<mspIdOrigen>_<mspIdDestino>`.
...
1. **Definición de cada colección**: para cada par ordenado (organización origen, organización destino) cuyo par de `agentType` esté autorizado por la matriz `domain/authorized-transfers.json`, se define una colección con:
   - nombre determinístico `transfer_<mspIdOrigen>_<mspIdDestino>`;
...
3. **Qué colección usa cada operación**: el despacho (`DispatchTransfer`) y la recepción/rechazo (`ReceiveTransfer`/`RejectTransfer`) usan la colección del par (organización origen del despacho, organización del destino declarado). Los eventos extraordinarios (T09, T13–T16) no escriben datos privados nuevos: su campo `motivo` es un argumento público del canal (contrato v2.0.2).
...
La convención de nombres determinística (`transfer_<mspIdOrigen>_<mspIdDestino>`) hace que el chaincode pueda resolver la colección de una operación sin tablas de mapeo adicionales: con el `mspId` del emisor (invocador del despacho) y el `mspId` del destino declarado (resuelto desde el registro de ADR-003), el nombre de la colección se computa determinísticamente en el endoso — condición necesaria para que todos los peers endosantes escriban en la misma colección.
