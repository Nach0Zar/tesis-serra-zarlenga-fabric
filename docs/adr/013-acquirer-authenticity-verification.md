...
3. **Cadena de custodia legítima.** La secuencia completa de estados del historial corresponde a un camino válido de la tabla de ADR-001 —cada par consecutivo es una transición declarada y el primer estado es `EN_LABORATORIO`—, y cada cambio de `CustodioActual` observado corresponde a un par (`agentType` origen → `agentType` destino) autorizado por la matriz embebida de ADR-008, resolviendo el `agentType` de cada custodio contra el registro organización-establecimiento vigente. Si falla lo primero, el veredicto es `SECUENCIA_INVALIDA`; si falla lo segundo, `TRANSFERENCIA_NO_AUTORIZADA`.

   Son las comprobaciones 4 y 5 de ADR-011, con idéntica semántica e idénticos veredictos nombrados. **Deben implementarse una sola vez y ser consumidas por ambas operaciones**; CC-7, que llega primero, deja los helpers, y CC-8 los usa. Dos implementaciones de la misma regla es exactamente lo que ADR-008 prohíbe para la matriz, por el mismo motivo: divergirían en silencio.
4. **Estado apto para operar.** El estado actual de la unidad no es **bloqueante** ni **terminal** según ADR-001. Si es bloqueante —`VENCIDO`, `DETERIORADO`, `RETIRADO_MERCADO`, `PROHIBIDO`, `DEVUELTO`, `EN_CUARENTENA`— el veredicto es `ESTADO_BLOQUEANTE`; si es terminal —`DISPENSADO`, `ROBADO`, `EXTRAVIADO`, `DISPUESTO_FINAL`— es `ESTADO_TERMINAL`. Ambos se reportan con el estado observado en el detalle.

   La separación en dos veredictos no es cosmética: un estado bloqueante puede resolverse —ADR-001 les conserva transiciones de salida— y un estado terminal no. Para el adquirente son dos decisiones distintas: rechazar la recepción a la espera de una resolución, o rechazarla definitivamente porque la unidad ya salió de circulación.
...
- **Para CC-8 (#62)**: las comprobaciones 4 y 5 de ADR-011 quedan implementadas y probadas por CC-7 como helpers compartidos. CC-8 las consume en lugar de reescribirlas; **no** debe volver a implementarlas.
