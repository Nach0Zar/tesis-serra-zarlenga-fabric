...
3. **Autoridad de certificación**: un **único proceso servidor de Fabric CA** que hospeda una **CA lógica por organización, cada una con su propia raíz de confianza**, configuradas con `cafiles` (Fabric CA admite múltiples signing CAs dentro de un mismo servidor). Emite las identidades de todas las organizaciones del prototipo — peers, orderers y clientes, con los atributos `snt.role` de DES-6 — pero **cada MSP declara como trust anchor únicamente la raíz de su propia CA lógica**.

   La distinción entre *proceso servidor* y *CA lógica* es deliberada y corrige una versión anterior de este ADR, que decidía «una única instancia de Fabric CA» sin separarlas. Compartir la **raíz** entre todas las MSP debilitaría exactamente la propiedad sobre la que se apoya ADR-003: ese ADR sostiene que la suplantación de un establecimiento por otro no requiere lógica de chaincode porque está cubierta por las garantías criptográficas de la membresía, y eso deja de ser cierto si un mismo certificado raíz respalda a todas las organizaciones. Con una raíz por organización, el límite criptográfico coincide con el límite de establecimiento que ADR-003 fija, y se conserva el ahorro de recursos que motivaba la decisión: un solo contenedor en el host de medición.

   **Límite de confianza declarado**: lo que sigue compartido es el **operador** del proceso, que tiene acceso al material de todas las CA lógicas y podría emitir identidades de cualquier organización. En producción cada organización operaría su propio servidor de CA (o CA intermedias bajo su control) bajo administración independiente; para el prototipo, el objetivo de la medición no depende de esa separación operativa, y la simplificación debe listarse entre las limitaciones del trabajo. Se descarta **cryptogen** para el material criptográfico definitivo de la red: no representa el proceso de enrolamiento contra una CA y no ejercita la emisión de atributos; puede usarse únicamente para pruebas locales descartables. Esto resuelve la pregunta abierta de NET-1 (#20).
...
5. **Bootstrap del dataset mínimo**: secuencia ordenada, ejecutada por los scripts de red (NET-2/NET-4):
   ...
   - (c) **secuencia 1 de lifecycle — despliegue de bootstrap**: el paquete del chaincode `snt` (que embebe la matriz de ADR-008 y el manifiesto fundacional de ADR-010) se **construye una sola vez** y se distribuye idéntico; instalación en cada peer, `approveformyorg` de cada organización, y `commit` de la definición con el `collections_config.json` generado conforme ADR-006, `--init-required` y **política de endoso `AND` de todas las organizaciones fundacionales** (ADR-010, punto 4);
   - (c bis) **verificación del artefacto instalado**, antes de invocar `Init`: `peer lifecycle chaincode queryinstalled` en cada peer y `queryapproved` por organización, contrastados contra el `packageID` y el checksum versionados en el repositorio. Es necesario porque el `packageID` **no** integra la definición confirmada en el canal —es un parámetro local de `approveformyorg` y Fabric no exige que coincida entre organizaciones (ADR-010, punto 4)—, de modo que la igualdad de binarios es un control de despliegue verificable y no una garantía de la plataforma. La salida queda como evidencia de NET-4 (#23);
   ...
   - (e) **secuencia 2 de lifecycle — definición operativa**: nueva secuencia con la misma versión de paquete y la **política de endoso operativa** (`OR` laxa de las organizaciones custodiales y de la regulatoria, punto 6.j);
...
6. **Materialización del endoso (para NET-6)**. El diseño combina tres mecanismos nativos de Fabric, cada uno con un alcance distinto: la política de endoso de **chaincode** (laxa, `OR` de las organizaciones custodiales y de la regulatoria — punto 6.j), el **state-based endorsement (SBE)** por clave pública, y la **política de endoso de las colecciones implícitas por organización**.
   ...
   **Marcador de participación.** Se define un patrón único, aplicable a toda regla de DES-6 que exija el endoso de una organización que no es la titular de la clave escrita: la transacción escribe, en la colección implícita de esa organización, un **marcador** con clave única por transacción y contenido determinístico calculado por el chaincode. La clave tiene la forma `Participacion`+[`<tipoObjeto>`, `<componentes del objeto>`, `txId`], con dos variantes: `Participacion`+[`Unidad`,`gtin`,`numeroSerie`,`txId`] cuando la operación recae sobre una unidad, y `Participacion`+[`Organizacion`,`mspId`,`txId`] cuando recae sobre una entrada del registro organización-establecimiento, que no tiene GTIN ni número de serie — una versión anterior de este ADR definía una sola forma, por unidad, que las operaciones registrales del punto 6.g no podían construir. El `txId` va último en ambas variantes: garantiza la unicidad por transacción y deja los componentes anteriores disponibles para `GetPrivateDataByPartialCompositeKey`. El valor registra la operación, el `mspId` del invocador y el timestamp de la transacción. El marcador no transporta información de negocio, no requiere `transient` ni intervención del cliente, y su único propósito es someter la transacción a la política de endoso de esa colección. Propiedades que lo hacen apto:

   - **fuerza el endoso desde la primera escritura**, sin la ventana que afecta a toda clave pública nueva;
   - **no serializa**: la clave es única por transacción, con lo que no hay contención MVCC — a diferencia de un contador o registro compartido por organización;
   ...
   e. **Intervención de un laboratorio no custodio** (retiro, recupero o disposición final sobre una unidad ajena; fila correspondiente de DES-6). Se resuelve en dos transacciones:
      - **Autorización previa**: `AuthorizeLabIntervention` (contrato DES-5), invocada por la organización regulatoria, escribe la clave pública `LabIntervention`+[`gtin`,`numeroSerie`] con SBE **`AnmatMSP`** y un marcador de participación en la colección implícita regulatoria — que fuerza el endoso del regulador ya en esa primera escritura, validada por la política de chaincode del punto 6.j. La SBE de la clave de autorización es de la organización regulatoria **solamente**: así la autoridad puede reemplazarla o corregirla sin depender de nadie (ver punto f).
      - **Ejercicio**: la operación del laboratorio lee la autorización, verifica laboratorio, operación, `estado=ACTIVA` y vigencia contra `GetTxTimestamp()`, la marca como `CONSUMIDA` y escribe marcadores de participación en **su propia** colección implícita y en la **regulatoria**. Endosantes exigidos por la plataforma: **laboratorio designado** (su colección), **organización regulatoria** (su colección y la SBE de la clave de autorización) y **custodio actual** (SBE de la clave de la unidad). El par «laboratorio invocante y `AnmatMSP`» de DES-6 queda impuesto como coendoso de peers; el endoso adicional del custodio es un endurecimiento coherente con la justificación de esa misma fila —no mutar un asset bajo custodia ajena sin validación— y se registra como salvedad en DES-6.

   f. **Vigencia, reemplazo y revocación de la autorización.** `expiraEn` es un campo que la **lógica** del chaincode evalúa contra `GetTxTimestamp()`: al vencer, la autorización deja de ser ejercible, pero la clave y su SBE siguen existiendo — el vencimiento no borra nada. De ahí se siguen dos consecuencias, y la segunda corrige un error de la versión anterior de este ADR.

      **La SBE de esa clave no puede incluir al laboratorio designado.** Si lo incluyera, reemplazar una autorización vencida por otra dirigida a un laboratorio distinto exigiría el endoso del laboratorio anterior, y bastaría con que ese laboratorio se negara, perdiera su peer o saliera del canal para que la unidad quedara **impedida de forma permanente** de recibir cualquier autorización nueva. Corrige una versión anterior de este ADR, que fijaba SBE `AND(laboratorio, AnmatMSP)` sobre esa clave y trataba el vencimiento como si liberase el permiso.

      **Dejar sin efecto una autorización vigente exige una operación propia, no un reemplazo.** La versión anterior de este ADR y del contrato sostenía que la autoridad podía anular una autorización reemitiéndola con un `expiraEn` ya alcanzado. Eso es imposible: la propia validación de `AuthorizeLabIntervention` exige que `expiraEn` sea **posterior** al `GetTxTimestamp()` de la transacción, de modo que esa reemisión sería rechazada con `INVALID_LAB_INTERVENTION`. La vía que quedaba —emitir otra autorización válida— sirve para *reasignar*, no para *anular*: hasta que venciera, una autorización emitida por error seguía siendo ejercible. Se agrega por eso `RevokeLabIntervention` al contrato (DES-5 v2.5.0): la invoca la organización regulatoria, marca la autorización como `REVOCADA` con timestamp y motivo, y escribe el marcador regulatorio. Se **conserva** el registro en lugar de borrar la clave, por dos razones: la traza de emisión y revocación es evidencia auditable de un acto administrativo, y borrar la clave devolvería la próxima autorización sobre esa unidad a la ventana de creación del punto 6.j en lugar de dejarla protegida por la SBE que ya existe.

      La autorización tiene entonces tres estados persistidos —`ACTIVA`, `CONSUMIDA`, `REVOCADA`— y una condición derivada, el vencimiento, que la lógica computa contra el timestamp de la transacción y **no** se persiste: persistirla exigiría una transacción que ninguna organización tiene motivo para enviar. El ejercicio exige `estado=ACTIVA` además de laboratorio, operación y vigencia. La clave sigue siendo una por unidad (`LabIntervention`+[`gtin`,`numeroSerie`]): una autorización nueva sobre la misma unidad reemplaza a la anterior y `GetHistoryForKey` conserva la secuencia completa, con lo que el historial permite distinguir una autorización vencida de una consumida y de una revocada.

   g. **Registro inicial de unidades (T01).** La clave pública de la unidad no existe todavía, con lo que su primera escritura no puede quedar cubierta por SBE. `RegisterUnit` escribe, además de la clave pública, un marcador de participación en la colección implícita del laboratorio invocante: la política de esa colección **exige el endoso del peer del laboratorio ya en esa primera transacción**. Con eso, la fila de DES-6 «registro inicial de unidades → laboratorio invocante» queda materializada por la plataforma, y no solo por la firma de creador. Precisión que corresponde conservar: la política de chaincode (`OR` de las custodiales y la regulatoria, punto 6.j) sigue admitiendo endosantes adicionales; lo que la plataforma garantiza es que el laboratorio es **necesario**, no que sea el único. Endosantes de más solo agregan ejecuciones coincidentes de la misma lógica determinística.

      La misma técnica cierra la ventana de creación de las claves de gobernanza que crea la organización regulatoria (`RegisterOrganization`, `SetOrganizationActive`, `AuthorizeLabIntervention`, `RevokeLabIntervention`): todas escriben un marcador en la colección implícita regulatoria — con la variante `Organizacion` de la clave del marcador en las dos primeras, que no recaen sobre una unidad. Para que el peer regulatorio **baste** en esas operaciones, la política de chaincode debe incluir a su organización: es el punto 6.j. `Init` no necesita marcador: se ejecuta bajo la política estricta `AND` de todas las organizaciones fundacionales (ADR-010).
   ...
   h. **Estados terminales.** Los terminales absolutos de ADR-001 son `DISPENSADO`, `ROBADO`, `EXTRAVIADO` y `DISPUESTO_FINAL`: en ellos la política queda en el valor de reposo del último custodio registrado y no se endurece, porque ADR-001 no admite transiciones de salida y es la lógica del chaincode la que rechaza el intento con `INVALID_STATE_TRANSITION`. Corrige una versión anterior de este ADR, que listaba `PROHIBIDO` como terminal —ADR-001 admite T23 hacia `DEVUELTO` y T32 hacia `DISPUESTO_FINAL`— y omitía `ROBADO` y `EXTRAVIADO`.

      `VENCIDO`, `DETERIORADO`, `RETIRADO_MERCADO`, `PROHIBIDO`, `DEVUELTO` y `EN_CUARENTENA` **bloquean la circulación ordinaria y la dispensación, pero no son terminales**: conservan la política de reposo del custodio registrado, que es lo que les permite resolverse por devolución, reingreso autorizado o disposición final.
   ...
   j. **Composición de la política de chaincode.** La política de endoso de la definición operativa (secuencia 2 del punto 5) es `OR(<organizaciones custodiales>, <organización regulatoria>)`. Incluir a la organización regulatoria no es una concesión sino un requisito de corrección que las versiones anteriores de este ADR omitieron. La política de chaincode rige **únicamente la primera escritura de una clave pública** —a partir de ahí rige la SBE de esa clave, que tiene precedencia sobre ella—, y dos de esas primeras escrituras son actos administrativos que DES-6 y el contrato declaran exclusivos del regulador: la creación de una entrada del registro organización-establecimiento (`RegisterOrganization`) y la creación de la clave de autorización de intervención (`AuthorizeLabIntervention`). Con una política que solo enumeraba a las custodiales, esas dos transacciones exigían —de forma accidental y no declarada en ningún documento— el endoso adicional de **una organización custodial cualquiera**: una droguería arbitraria coendosando el alta registral de otra organización. Ninguna regla de DES-6 lo pide y ninguna consideración de seguridad lo justifica.

      **Por qué ampliarla no reabre el bypass regulatorio que descarta el punto 6.a.** Aquella rama disyuntiva estaba en la política de **la clave de una unidad**, y por eso alcanzaba a todas las operaciones ordinarias sobre esa clave. La política de chaincode no alcanza a ninguna de ellas: toda unidad tiene SBE desde T01 y la SBE tiene precedencia. Lo que la política de chaincode gobierna es un conjunto cerrado de tres primeras escrituras, y en las tres el diseño ya fija por otro mecanismo qué organización queda exigida:

      | Primera escritura de una clave pública | Organización exigida, y por qué mecanismo |
      |---|---|
      | `RegisterUnit` (clave de la unidad) | Laboratorio invocante, por su marcador de participación (punto 6.g) |
      | `RegisterOrganization` (entrada del registro) | Organización regulatoria, por su marcador |
      | `AuthorizeLabIntervention` (clave de autorización) | Organización regulatoria, por su marcador |

      **Invariante de diseño que lo sostiene**: *toda* escritura de una clave pública nueva va acompañada del marcador de participación de la organización responsable de esa escritura. Mientras se cumpla, la política de chaincode no es una frontera de seguridad —solo exige que al menos un peer de la red haya ejecutado la lógica— y la garantía efectiva la dan los marcadores y la SBE. CC-1 debe tratarla como invariante verificable y no como detalle de implementación: cualquier operación futura que cree una clave pública sin marcador reabriría una ventana de creación sin dueño.

      **Pruebas de aceptación (NET-6, #25)**, en los dos sentidos:

      - `RegisterOrganization` endosada **solo** por el peer regulatorio → válida;
      - `AuthorizeLabIntervention` y `RevokeLabIntervention` endosadas **solo** por el peer regulatorio → válidas;
      - `RegisterUnit` endosada solo por el peer regulatorio → `ENDORSEMENT_POLICY_FAILURE` (falta el marcador del laboratorio);
      - `Dispense` endosada solo por el peer regulatorio → `ENDORSEMENT_POLICY_FAILURE` (falta la SBE de reposo del custodio);
      - `ReceiveTransfer` endosada por el receptor y el peer regulatorio, sin el emisor → `ENDORSEMENT_POLICY_FAILURE`.

      `Init` queda fuera de este punto: se ejecuta en la secuencia 1, bajo la política `AND` de todas las organizaciones fundacionales (ADR-010).
...
LevelDB y el **proceso servidor compartido de Fabric CA** responden al mismo criterio: no cargar el host único de medición con componentes que el contrato congelado y el objetivo de la medición no necesitan. El servidor hospeda una CA lógica con raíz propia por organización, de modo que no se comparte el *trust anchor* entre MSP; la simplificación que se declara como límite es **operativa**: un mismo operador controla el proceso y el material de todas esas CA y podría emitir identidades para cualquiera de ellas. En producción, cada organización administraría su propio servidor o su propia CA intermedia bajo control independiente. En el caso de LevelDB, la simplificación es reversible por revisión de este ADR y no toca ninguna garantía.
...
- **La política de chaincode también es parte de la materialización.** Resueltos los tres anteriores, quedó una consecuencia no advertida: la política laxa enumeraba solo a las organizaciones custodiales, con lo que las dos primeras escrituras que el diseño reserva al regulador exigían además el endoso de una custodial cualquiera. El punto 6.j la amplía a `OR(custodiales, regulatoria)` y deja explícito qué gobierna esa política —el conjunto cerrado de primeras escrituras— y qué no.
- **Un campo de vigencia no es un mecanismo de revocación.** El ADR y el contrato afirmaban que la autoridad podía anular una autorización reemitiéndola vencida, pero la validación de la propia operación exige una fecha futura y habría rechazado esa reemisión. El punto 6.f agrega `RevokeLabIntervention` como operación real.
...
- **NET-3 (#22)**: el dimensionamiento del host debe presupuestar 3 orderers, los peers del punto 2 y un proceso servidor de Fabric CA que hospeda las CA lógicas del punto 3; LevelDB evita contenedores CouchDB adicionales.
- **NET-4 (#23)**: los scripts de despliegue implementan la secuencia de bootstrap del punto 5, con la política operativa `OR(custodiales, regulatoria)` del punto 6.j en la secuencia 2; los workflows de CI migran de `mychannel` a `snt-channel` cuando la red propia reemplace a `test-network`.
- **NET-6 (#25)**: la demo de endoso debe evidenciar el rechazo por la plataforma de: (i) una recepción o un rechazo endosados por una sola de las dos partes del tránsito; (ii) una dispensación endosada por un peer que no sea el del custodio; (iii) un evento regulatorio sin el endoso del peer regulatorio, o sin el del custodio; (iv) una operación de laboratorio no custodio sin el endoso del laboratorio, del regulador o del custodio; (v) un `RegisterUnit` sin el endoso del peer del laboratorio invocante; y, en el sentido inverso, la validez de `RegisterOrganization`, `AuthorizeLabIntervention` y `RevokeLabIntervention` endosadas únicamente por el peer regulatorio (punto 6.j). Debe además demostrar la restauración de la política de reposo en las tres salidas de `EN_TRANSITO` (punto 6.c) y separar el rechazo por política (`ENDORSEMENT_POLICY_FAILURE`) del rechazo por lógica (error tipificado del contrato). La afirmación que la evidencia debe sostener es la **acotada** de la Justificación, no la versión amplia.
...
- **Para el contrato DES-5**: el punto 6 fija la superficie de `AuthorizeLabIntervention` (SBE regulatoria sobre su clave, vigencia evaluada por lógica, `estado` persistido) y de `RevokeLabIntervention` (punto 6.f), y obliga a documentar, por operación, qué organizaciones deben endosar.
...
- **Se gana**: correspondencia estructural con la afirmación de descentralización del ordenamiento del trabajo escrito (verificable en la configuración del canal, no en la medición); parámetros concretos que desbloquean NET-1/2/3/4/6; endoso multiparte exigido por la plataforma en los cuatro casos que DES-6 lo pide, incluido el registro inicial que ninguna versión anterior lograba cubrir; y la propiedad acotada que la Justificación enuncia: **ninguna organización puede, por sí sola, cambiar la custodia de una unidad, intervenir sobre una unidad ajena ni sustituir a una contraparte exigida**; menor huella de recursos en el host de medición (LevelDB y un solo proceso servidor de Fabric CA) sin compartir la raíz de confianza entre MSP.
- **Se pierde / costo**: la configuración de ordenamiento multi-organización es más compleja que el default de `test-network` (tres MSP de orderer, material criptográfico y consenters por organización); un mismo operador controla el material de todas las CA lógicas, aunque cada MSP confía únicamente en su propia raíz; LevelDB cierra la puerta a rich queries hasta una revisión de este ADR; toda escritura sobre una unidad ajena queda subordinada a la disponibilidad del peer del custodio actual —o de ambas partes durante el tránsito—, y los eventos regulatorios agregan la del peer regulatorio; y cada alta y cada evento regulatorio pagan una escritura privada adicional (punto 6.g).
...
- **Servidor Fabric CA compartido**: cada organización conserva una CA lógica y una raíz de confianza propia, pero todas se hospedan en un único proceso administrado por el mismo operador. La simplificación respecto de una red productiva es, por lo tanto, de **independencia operativa**, no de separación de *trust anchors*: en producción cada organización operaría su propio servidor o una CA intermedia bajo control independiente. Este límite debe quedar explícito en la próxima iteración del trabajo escrito.
