...
2. **Protocolo de medición**: la sección 8.1 de `docs/measurement-protocol.md` asume un cluster Raft de 3 orderers, con caída de 1 (Raft-1: la red conserva quorum) y de 2 (Raft-2: la red pierde quorum y deja de ordenar). La topología debe hacer que esos escenarios midan algo con significado, no solo la tolerancia a fallas de contenedores intercambiables.
...
- Da significado real a los escenarios Raft-1/Raft-2 del protocolo de medición: la caída de un orderer es la caída del nodo de una organización, no de un contenedor más de un mismo administrador.
...
- La prueba de disponibilidad Raft seguiría siendo válida como demostración de tolerancia a fallas (CFT), pero la afirmación de descentralización administrativa del trabajo escrito quedaría sin respaldo experimental: los tres nodos dependerían de un único administrador.
...
- Se descarta porque el costo de configuración de la alternativa A es acotado y pagable, mientras que el costo de B es argumental y permanente: debilitaría uno de los tres pilares de la hipótesis justo en el punto donde el prototipo puede demostrarlo.
...
La decisión de ordenamiento es la única de las seis con riesgo argumental. El trabajo escrito sostiene que distintas organizaciones contribuyen nodos al servicio de ordenamiento, reduciendo la dependencia de un único administrador central, y usa la eliminación del punto único de falla como argumento de la hipótesis. Repartir los 3 orderers entre `AnmatMSP`, `LabMSP` y `DrogueriaMSP` materializa esa afirmación en lugar de simplificarla: los escenarios Raft-1 y Raft-2 del protocolo de medición pasan a demostrar que la red sobrevive a la caída del nodo de una organización completa y que la pérdida de quorum requiere la caída de dos organizaciones distintas — que es la propiedad de descentralización administrativa que la tesis afirma, no solo la CFT genérica de Raft. La alternativa de organización única habría exigido acotar esa conclusión en la tesis a cambio de un ahorro de configuración modesto; se prefiere pagar el costo de configuración.
...
- **Protocolo de medición (§8.1)**: los escenarios Raft-1/Raft-2 conservan validez y ganan significado: cada caída de orderer es la caída del nodo de una organización distinta.
- **Se gana**: respaldo experimental para la afirmación de descentralización administrativa del trabajo escrito; parámetros concretos que desbloquean NET-1/2/3/4/6; endoso multiparte exigido por la plataforma (evidencia para NET-6); menor huella de recursos en el host de medición (LevelDB, CA única).
...
- **Servicio de ordenamiento**: **no hay divergencia**. Se eligió deliberadamente la opción fiel a la afirmación del trabajo escrito ("distintas organizaciones contribuyen nodos al servicio de ordenamiento, reduciendo la dependencia de un único administrador central"); la topología adoptada la materializa y los escenarios Raft del protocolo de medición la respaldan experimentalmente.
