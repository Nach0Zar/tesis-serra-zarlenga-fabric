...
4. **Bootstrap de la entrada del regulador.** Extender el registro a los no custodiales crea un problema de huevo y gallina: `RegisterOrganization` exige `agentType=REGULATOR`, pero la primera entrada `REGULATOR` no puede haber sido registrada por nadie. Se resuelve anclando esa primera entrada a un artefacto que las organizaciones **aprueban administrativamente**, más una secuencia de despliegue en dos etapas y tres invariantes permanentes.
   ...
   **Raíz de confianza: el paquete de chaincode aprobado.** Lo que en el lifecycle de Fabric sí firma cada organización con su propia identidad de administrador es el `approveformyorg` de una definición que incluye un `packageID` determinado — el hash del paquete instalado. En consecuencia, un valor contenido **dentro del paquete** queda aprobado por cada organización de forma verificable: aprobar otro contenido exige aprobar otro `packageID`, y la definición confirmada en el canal deja constancia de cuál fue.
   ...
   - El chaincode **embebe** ese manifiesto por `go:embed`, con el mismo mecanismo y la misma justificación que ADR-008 fijó para la matriz de transferencias: lo que se aprueba en el lifecycle contiene literalmente las reglas y los datos que se evalúan, y todos los peers endosantes ejecutan el mismo binario. El layout Go concreto se resuelve donde ADR-008 lo dejó (CC-1).
   - **`Init` no recibe el `mspId` regulatorio como argumento.** Resuelve la identidad del regulador exigiendo, de forma conjunta, tres condiciones:
     ...
     Sembrar un regulador distinto del declarado deja de ser posible sin cambiar el paquete, y cambiar el paquete exige un `approveformyorg` nuevo de cada organización. La firma concreta de la operación la fija el contrato DES-5 (`Init`), no las issues de implementación.
...
- **Queda pendiente**: el esquema del manifiesto fundacional (NET-2) y el procedimiento de recuperación si la semilla se confirmara con un paquete equivocado — que exige repetir el ciclo de lifecycle con un `packageID` nuevo.
...
- [ADR-008: Distribución de la matriz de transferencias autorizadas](008-transfer-matrix-distribution.md): mecanismo `go:embed` + paquete compartido y su justificación (lo aprobado en el lifecycle contiene literalmente lo que se evalúa), reutilizado acá para el manifiesto fundacional.
