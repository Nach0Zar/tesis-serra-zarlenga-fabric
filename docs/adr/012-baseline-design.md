...
Lo que ningún documento había decidido formalmente es el diseño de esa baseline: stack, esquema, emulación de identidad y — sobre todo — qué significa en concreto la "paridad funcional" que el protocolo da por sentada. Sin ese checklist explícito, cualquier diferencia de resultados en la evaluación es atacable con "compararon contra una baseline mal hecha" (`docs/adr-roadmap.md`, decisión D7). Piezas del rompecabezas ya estaban fijadas por decisiones previas: el contrato del chaincode (`docs/api-contract.md`, v2.0.2) define el catálogo de códigos de error estables sobre los que "el cliente y la baseline deben ramificar"; ADR-001 obliga a que "el chaincode y la baseline" rechacen toda transición no declarada; `domain/README.md` prohíbe duplicar la matriz de transferencias en condicionales mantenidos a mano; y ADR-008 ya decidió que chaincode y baseline consumen la matriz a través de un paquete Go compartido del repositorio, anticipando que la baseline sería Go "conforme ADR-012/D7". El `baseline/README.md` recoge esos tres mandatos como requisitos del directorio aún vacío.
...
|---|---|---|
| `organizations` | Espejo del registro organización-establecimiento de ADR-003: identificador canónico (`GLN:`/`CUFE:`), `idType`, `agentType` — incluidos los tipos no custodiales que incorpora ADR-010 — y `active`. | Registro organización-establecimiento del ledger (ADR-003). |
...
| `unit_events` | Historial **append-only**: cada operación de escritura inserta un evento con timestamp, operación, invocador y estado resultante. Nunca se actualiza ni se borra una fila. | Análogo funcional del transaction log de Fabric. |
| `transfer_operations` | Registro de operación de transferencia: destinatario declarado, remito/factura, `ruleId` (y versión de matriz, conforme ADR-008), estado activo/cerrado. | Registro de operación en la PDC del par (ADR-006), con la **diferencia declarada** de que acá no existe confidencialidad real entre "organizaciones": todas las filas conviven en la misma base administrada por un único operador. |
...
- [ ] **Misma verificación del financiador**: la baseline expone la verificación de traza con el mismo veredicto estructurado que define ADR-011.
- [ ] **Mismos endpoints conceptuales** que las operaciones del contrato: registro, despacho, recepción, rechazo, dispensación, eventos extraordinarios, consulta puntual y consulta de historial (los nombres REST concretos los define BASE-2).
...
- **Endoso multiorganizacional**: en la baseline valida un único proceso; no hay coendoso emisor+receptor ni coendoso regulatorio.
...
- **BASE-1 (#37)**: desbloqueada; materializa el esquema de la sección 2 como DDL con migraciones versionadas e índices equivalentes a las consultas del chaincode.
...
- [`docs/api-contract.md`](../api-contract.md) (v2.0.2): catálogo de códigos de error estables y operaciones del contrato que la baseline debe espejar.
...
- ADR-010 (Identidad de las organizaciones no custodiales, decisión D5/issue #85): `agentType` no custodiales que `organizations` debe admitir.
- ADR-011 (Criterios de verificación de traza del financiador, decisión D6/issue #86): veredicto estructurado que la baseline expone con la misma semántica.
