...
- [ ] **Mismos endpoints conceptuales** que las operaciones del contrato: registro de unidades, despacho, recepción, rechazo, dispensación, eventos extraordinarios, autorización y revocación de intervención de laboratorio, administración del registro organización-establecimiento, consulta puntual, consulta de historial y verificación de traza (los nombres REST concretos los define BASE-2). La inicialización (`Init`) no tiene análogo: en la baseline el seed del registro es una migración, no una transacción bajo política de endoso.
