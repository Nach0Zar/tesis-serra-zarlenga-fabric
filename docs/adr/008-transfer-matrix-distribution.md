...
1. **Distribución por `go:embed`.** El chaincode embebe `domain/authorized-transfers.json` en su binario en tiempo de compilación mediante la directiva `//go:embed`, lo parsea una única vez en la inicialización del contrato y evalúa cada despacho con el algoritmo exacto de `domain/README.md`: coincidencia exacta en `authorizedTransfers` → autorizar con el `ruleId` de la regla; coincidencia con una prohibición explícita → denegar con el `id` de la prohibición; en cualquier otro caso → denegar con razón `DEFAULT_DENY`. El determinismo de endoso queda garantizado por construcción: todos los peers endosantes ejecutan el mismo binario, y por lo tanto la misma matriz.
...
- **Determinismo primero.** La propiedad no negociable es que todos los endosantes de `DispatchTransfer` lleguen a la misma decisión. Embeber la matriz en el binario hace de esa propiedad una consecuencia del empaquetado del chaincode — el mismo paquete aprobado por las organizaciones en el ciclo de vida contiene las reglas que se evalúan — en lugar de una propiedad operativa a mantener (alternativa C) o de un estado a sincronizar (alternativa A).
...
- **Para CC-3 (#16) y BASE-2 (#38)**: ambas implementaciones consumen el paquete compartido de `domain/`; ninguna define pares por su cuenta. Los tests de paridad pueden ejercitar la misma función contra los mismos casos.
...
- **Se pierde / costo**: actualizar la matriz exige recompilar y hacer upgrade del chaincode y rebuild de la baseline; la re-evaluación histórica solo es válida mientras la matriz no cambie entre eventos (limitación declarada de v1).
